...
def _parse_inference_output_for_format(output: str) -> List[str]:
    ...
    for line in lines:
        ...
        if line.startswith("[END]"):
            end_blocks += 1
            for token in ("success=", "steps=", "score=", "rewards="):
                if token not in line:
            ...
            score_part = line.split("score=", 1)[1].split(" ", 1)[0] if "score=" in line else ""
            if score_part and not re.fullmatch(r"\d+\.\d{2}", score_part):
                errors.append(f"[END] score must be 2-decimal format: {line}")
